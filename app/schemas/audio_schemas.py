...
class MismatchedAudio(BaseModel):
    """Model for a single mismatched audio."""
    
    audio_filename: str
    database_duration: Optional[float] = None  # Duration in seconds from database
    actual_duration: Optional[float] = None  # Actual duration in seconds from cloud


class VideoMismatchInfo(BaseModel):
    """Model for video with mismatched audios."""
    
    video_id: str
    correct_audio_count: int
    mismatch_audio_count: int
    mismatched_audios: List[MismatchedAudio]


class AudioDurationMismatchResponse(BaseModel):
    """Response model for audio duration mismatch check."""
    
    total_mismatch_audios: int
    total_relevant_videos: int
    videos: List[VideoMismatchInfo]
...
class AudioDeleteResponse(BaseModel):
    ...
    total_deleted: int
    total_failed: int
    deleted_audios: List[str] = Field(default_factory=list, description="List of successfully deleted audio filenames")
    failed_audios: List[Dict[str, str]] = Field(default_factory=list, description="List of failed audio filenames with reasons")